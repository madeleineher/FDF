	...
	while (++y < e->pla.ly)
	{
		...
		while (++x < e->pla.lx)
		{
			...
			e->co[y][x].x2 = (x - y) * (e->spax / 2)
				+ e->pla.px + e->iso + e->ml + e->mx; //+ e->rl + e->rr;
			e->co[y][x].y2 = (x + y) * (e->spay / 2)
				+ e->pla.py + e->ml + e->my - (e->co[y][x].z * e->hi); //+ e->rl + e->rr;
		}
	}
}
...
void	projection(t_env *e)
{
	...
	if (e->iso_check == 1)
	{
		while (++y < e->pla.ly)
		{
			...
			while (++x < e->pla.lx)
			{
				e->co[y][x].x2 = (e->pla.px + x * e->spax) + e->ml + e->mx; //+ e->rl + e->rr;
				e->co[y][x].y2 = (e->pla.py + y * e->spay)
					+ e->ml + e->my - (e->co[y][x].z * e->hi); //+ e->rl + e->rr;
			}
		}
	}
